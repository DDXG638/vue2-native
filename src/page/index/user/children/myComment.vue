                        ...
                        <ul class="list">
                            <li v-for="(item,index) in json" :key='index'>
                                ...
                                <div class="mid">
                                    ...
                                    <router-link :to='url(item.classid, item._id)'>
                                        <div class="article">
                                            <div class='news_img'>
    ...
    methods: {
        ...
        deleteComment(item) {
            ...
            .then(() => {
                ...
                this.delete_myComment_data({newsDetailId: item._id, commentId: item.commentId})
                .then(res => {
                    if (res.code === 0) {
                        this.$toast({message: '删除成功', duration: 1500})
                        let index = this.json.findIndex(n => n.commentId === item.commentId)
        ...
        url(classid, _id) {
            return `/detail?classid=${classid}&id=${_id}`
        }
    },
    mounted() {
        this.headerFixed()
...
<style>
#myComment .cover_bg {
    background: url(~@/assets/img/cover_bg1.jpg)no-repeat center center;
